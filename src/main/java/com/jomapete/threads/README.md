...
It can perform complicated calculation or to execute complex tasks in the background without interrupting the main program.
...
Thread.startVirtualThread(() -> { /* tarea */ });
...
· Classic way: you can straight forward use the Thread class or use classes that extend Thread.
    Thread legacyThread = new Thread(() -> { /* tarea */ });
    legacyThread.start();
